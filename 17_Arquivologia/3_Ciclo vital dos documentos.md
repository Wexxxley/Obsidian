
#Concluded 


---
**Estágios de evolução dos arquivos:** Arquivos que guardam documentos...
1. **Corrente/1°idade:** mais novos que ainda estão sendo resolvidos, frequentemente consultados. Normalmente ficam junto ao setor ou próximo
2. **Intermediário/temporário/2°idade:** que já foram resolvidos mas podem ser questionados. Não tem necessidade de ser intalado junto à instituição, normalmente são galpões, por questões de espaço e aconomia. Podem ser chamados de Limbo ou Purgatório. 
3. **Permanente/3°idade:** que perderam valor administrativo e ganharam valor histórico. Nunca são eliminados e não possuem restrição de acesso.
 
Valores dos documentos:
1. **Primário ou administrativo:** Compreende as fases correntes e intermediárias.
2. **Secundário ou histórico:** Compreende a fase permanente. Poucos documentos atingirão este valor. Valor imprescritível, probatório ou informativo.

No dia a dia é necessário eliminar alguns arquivos periodicamente. Alguns não podem ser eliminados, estes tornam-se permanentes.

**Avaliação dos documentos:** análise dos documentos a fim de definir prazos de guarda e destinação final. Realizada por um grupo **multidisciplinar** (comissão de avaliação dos documentos) e resulta na **Tabela de temporalidade da instituição**. 
![](../attachments/Pasted%20image%2020260527090538.png)
Existe uma legislação que **exige que todo órgão público tenha esse controle.**

**Documentos que não podem ser eliminados:**
- Documentos que provam como a instituição foi organizada: diretrizes, normas, relatórios de atividades, etc

**Documentos que podem ser eliminados com o tempo:**
- Documentos cujos textos ou elementos essenciais estiverem reproduzidos em outros documentos. Como cópias.
- Documentos de pura formalidade, como convites, cartas de agradecimento.
- Documentos absoletos.

![](../attachments/Pasted%20image%2020260527094414.png)
![](../attachments/Pasted%20image%2020260527094430.png)

---

**Plano de destinação:** Instrumento utilizado para o tratamento de **massas documentais acumuladas**, ou seja, o acúmulo de documentos sem que tenham sido submetidos a qualquer tratamento arquivístico adequado. Após esse palno alguns documentos serão armazenados permanentemente, outros eliminados
![350](../attachments/Pasted%20image%2020260527092557.png)

**Transferência**: entrada de documentos no arquivo intemediário.

**Recolhimento**: entrada de documentos do arquivo permanente.

![](../attachments/Pasted%20image%2020260527093810.png)
Todo documento passa pela idade corrente. Nem todo documento passa pela idade intermediária e permanente. Alguns são eliminados já na fase corrente. 


![](../attachments/Pasted%20image%2020260527094335.png)
![](../attachments/Pasted%20image%2020260527094351.png)