...
---
Code Smells são **indicadores** de que um trecho de código pode precisar de refatoração. Note que os code smells estão ligado aos principios SOLID.  [[1_SOLID|SOLID]]
...
8. **Objetos Mutáveis:** Objetos cujo estado pode ser modificado após a criação. A crítica não é sobre a existência da mutabilidade em si, mas sobre o **uso desnecessário** dela.
	- A crítica é direcionada principalmente a **objetos que representam valores simples**. O livro sugere que classes como `CEP`, `Moeda`, `Endereco`, `Data`, `Hora`, `Fone` e `Email` deveriam, preferencialmente, ser imutáveis.
	    
9. **Classes de Dados:** Classes que possuem apenas atributos, mas nenhuma lógica. 
    
10. **Comentários:** Comentários podem ser um smell quando são usados para explicar um código ruim. A recomendação é: "Não comente código ruim, reescreva-o" . 