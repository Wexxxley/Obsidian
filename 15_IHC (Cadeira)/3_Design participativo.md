
#Concluded 

---
### **1. Objeto de Estudo**

- O Design Participativo (DP) tem raízes fortes em movimentos sociais e trabalhistas e surgiu na Escandinávia entre o final dos anos 60 e o início dos anos 70.
- O objetivo era <mark style="background: #ADCCFFA6;">melhorar a comunicação e a interação na indústria, garantindo que os trabalhadores tivessem controle democrático sobre as mudanças</mark> em suas ferramentas, ambiente e relações de trabalho.
- A partir dos anos 80, o DP passou a ser uma **metodologia de design** focada em valorizar o **conhecimento tácito** (conhecimento advidno de experiência, dificil de formalizar).

---
### **2. Usuario como Coprojetista**

"<mark style="background: #ADCCFFA6;">Design COM o Usuário, e não apenas Design PARA o Usuário</mark>".

- **Design para o usuario**: Usuários são apenas fonte de dados para que os designers tomem decisões de forma hierarquizada e autoritária.

- **Design participativo:** Usuários se envolvem e participam ativamente.
	- No DP, os usuários devem atuar como coprojetistas
	- O usuário final deve dar contribuições em todo o ciclo de vida do sistema.

---
### **3. Motivações para Adotar o Design Participativo**

#### **A. Aspecto Político e Sociológico**
- Usuários devem ter o poder de influenciar o próprio local de trabalho.
- A atividade de design carrega a responsabilidade de melhorar a qualidade de vida dos indivíduos impactados pelo sistema.
#### **B. Aspecto Contextual**
- Usuários e designers compreendem o mundo de maneira diferente. O usuário traz a perspectiva prática e contextual do dia a dia, que é inacessível ao designer.
#### **C. Aspecto Pragmático**
- Contribui para o aumento da eficiencia, qualidade, aceitação e frequência de uso do sistema.
- Ajuda a evitar a implementação de funções sem utilidade real.

**Benefícios**
- Aproximação entre a equipe de desenvolvimento e os usuários.
- Levantamento de necessidades e requisitos funcionais mais detalhados nas fases iniciais.
- Redução de erros de interpretação e entendimento pelos desenvolvedores.

---
## **4. Níveis de Envolvimento do Usuário**

| **Categoria**           | **Controle do Usuário**                         | **Exemplo de Papel do Usuário**                      |
| ----------------------- | ----------------------------------------------- | ---------------------------------------------------- |
| **Nenhum Envolvimento** |                                                 | O produto é imposto.                                 |
| **Simbólico**           | A participação é solicitada, mas ignorada.      | O designer finge ouvir, mas já tem a solução pronta. |
| **Pela Consulta**       | O usuário é consultado.                         | O usuário é uma fonte de informação passiva.         |
| **Fraco Controle**      | Pouca responsabilidade em cada fase do projeto. | O usuário é um revisor ocasional.                    |
| **Pela Atividade**      | Membro ativo da equipe ou ligação oficial.      | O usuário ajuda a definir as funcionalidades.        |
| **Forte Controle**      | Os usuários exercem o controle do projeto.      | Os usuários vetam ou aprovam todas as mudanças.      |
O Design Participativo visa atingir a participação **Pela Atividade** ou **Forte Controle**.

---
## 5. Seleção dos Colaboradores no DP

- **Risco de Acomodação:** Em projetos longos, há um risco de os usuários envolvidos passarem a ter um **modelo mental semelhante ao dos designers**.
- **Usuários não são Designers:** Não se deve esperar que o usuário gere ideias de design do zero. Eles são **ótimos em reagir** a soluções que não gostam ou que não funcionariam. 
- **Designers não são Usuários:** O designer não deve pressupor que algo intuitivo para ele será intuitivo para o usuário.

---
## **6. Estágios Básicos em Pesquisa**

#### **A. Exploração Inicial do Trabalho**
O designer-pesquisador se familiariza com os participantes, os ambientes e as ferramentas de trabalho. Envolve observação, entrevistas, visitas organizacionais.
#### **B. Processo de Descoberta**
Designers e participantes utilizam técnicas para compreender e priorizar a organização do trabalho e esclarecer objetivos e valores.   Incluem jogos organizacionais, _storyboards_, modelos de fluxo de trabalho.
#### **C. Prototipação** 
Designers e participantes modelam iterativamente os artefatos tecnológicos.  A prototipação ocorre várias vezes até que o sistema esteja de acordo com os objetivos. São necessárias avaliações.

---
## **7. Técnicas de Inclusão do Usuários no Projeto**

#### **A. Técnicas de Descoberta**
- **Questionário e Entrevistas**
- **Análise Contextual**
#### **B. Técnicas de Co-criação**
- **Brainstorming**    
- **Prototipagem em Papel**
#### **C. Técnicas de Estrutura e Avaliação**
- **Diagrama de Afinidades:** Para organizar ideias e dados de forma colaborativa.
- **Cardsorting**
- **Teste de Usabilidade**.
- **Avaliação Cooperativa**.