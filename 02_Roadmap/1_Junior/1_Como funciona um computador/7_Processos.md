...
- Quando você clica duas vezes no ícone do Chrome, o arquivo chrome.exe é carregado na RAM. <mark style="background: #ADCCFFA6;">Esse "programa vivo" na RAM é um processo.</mark>
- O sistema operacional (SO) garante que um processo não possa, por acidente ou malícia, acessar a memória de outro processo.
...
1. **Memória Virtual:** O processo acha que tem a memória RAM toda para ele (ilusão criada pelo SO). Esse espaço é dividido em:
    ...
    - **Stack:** Uma área para guardar variáveis locais de funções e controlar a execução (quem chamou quem).
...
- **Running:** A CPU está ativamente executando suas instruções agora. 
- **Waiting/Blocked:** Ele parou porque precisa de algo externo. 
