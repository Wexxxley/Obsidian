

1) Tema, ou possíveis temas.  
Tema não é apenas uma área. Procure detalhar mais.

- Plataforma colaborativa para estruturação de trilhas de aprendizado

1) Possível(is) questão(ões) de pesquisa ou hipótese: escreva uma pergunta que expresse aquilo que quer investigar ou uma hipótese a ser verificada.

- Como estruturar o armazenamento de Grafos Direcionados Acíclicos em banco de dados?
- Quais os algorítmos de sincronização e diferenciação em grafos possíveis para sistemas colaborativos?
- Como funciona os sistemas de relevância e ranqueamento?

3) Nome do orientador(a).

- David Sena Oliveira

4) Nome e Link para a principal CONFERÊNCIA relacionada ao seu tema.  
Indique o link para a página da conferência que contém a lista dos trabalhos apresentados na última edição do evento.

- Anais do XXI Simpósio Brasileiro de Sistemas Colaborativos: https://sol.sbc.org.br/index.php/sbsc/issue/view/1693

5) Nome e Link para um PERIÓDICO / JOURNAL / REVISTA que pode estar relacionado ao seu tema.  Apresente um link para a página que contém a lista dos trabalhos da última edição.

- JSERD: Journal of Software Engineering Research and Development: https://journals-sol.sbc.org.br/index.php/jserd/
---------------------

OBSERVAÇÃO 1: Se o evento não for científico e não possuir artigos científicos, não será considerado.

OBSERVAÇÃO 2: Sugestão e verificar na SBC-OpenLib (SOL) - [https://sol.sbc.org.br/](https://sol.sbc.org.br/) artigos de conferências e periódicos. Lá tem muitas opções. Mas informe os dados do evento / periódico.

OBSERVAÇÃO 3: Não é para informar um artigo, e sim onde ele foi publicado.
