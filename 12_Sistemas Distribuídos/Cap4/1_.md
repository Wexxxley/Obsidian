...
1. **Endereçamento e Associação:** Para que um processo possa receber mensagens, seu soquete deve estar associado a dois elementos essenciais:
	 - (Uma porta, O endereço IP do processo)

2. **Exclusividade da Porta:** Mensagens enviadas a um endereço IP e número de porta específicos só podem ser recebidas pelo processo cujo soquete esteja associado àquele endereço IP e porta. Um processo pode usar o mesmo soquete para enviar e receber mensagens, e embora um processo possa usar várias portas para receber mensagens, ele **não pode compartilhar portas** com outros processos no mesmo computador (exceto para processos que utilizam multicast IP).
...
Soquetes em Diferentes Protocolos

A forma como o soquete é utilizado difere ligeiramente dependendo do protocolo de comunicação.

1. Comunicação por Datagrama (UDP)

• O soquete UDP é usado para enviar e receber datagramas **sem garantia de entrega** (sem confirmações ou novas tentativas de envio).

• Um servidor associa seu soquete a uma **porta de serviço conhecida** para que os clientes possam enviar mensagens para lá.

• O cliente vincula seu soquete a qualquer porta local livre.
...
2. Comunicação por Fluxo (TCP)

• O soquete de fluxo (stream) é usado para estabelecer uma **conexão** orientada à conexão.

• **Papéis Definidos:** Na conexão, um processo atua como **cliente** e o outro como **servidor**.

• **Servidor (Escuta):** O servidor cria um **soquete de "escuta"** (`listen socket`), vinculado à porta de serviço, para esperar que os clientes solicitem conexões. O soquete de escuta mantém uma fila de pedidos de conexão recebidos.

• **Criação de Novo Soquete:** Quando o servidor aceita uma conexão, um **novo soquete de fluxo é criado** especificamente para se comunicar com o cliente, enquanto o soquete de escuta permanece na porta de serviço para aceitar outros pedidos.

• **Fluxos:** O par de soquetes cliente-servidor é conectado por **dois fluxos**, um de entrada e um de saída, permitindo que cada soquete tenha um fluxo em cada direção.
