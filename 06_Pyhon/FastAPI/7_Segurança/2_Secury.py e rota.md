...
- `create_access_token` é responsável por criar um novo token JWT. Ela recebe um dicionário de dados, adiciona um tempo de expiração ao token. Esses dados, formam o **payload** do JWT. Em seguida, usa a biblioteca `pyjwt` para codificar essas informações em um token.
