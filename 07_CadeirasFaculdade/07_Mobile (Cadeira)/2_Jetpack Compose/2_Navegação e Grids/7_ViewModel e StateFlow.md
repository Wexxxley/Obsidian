...
A vantagem central ViewModel é a sua capacidade de sobreviver às mudanças de configuração do sistema.
...
- **Comportamento Frio:** A execução da lógica (como acessar um banco de dados) permanece inerte e só é iniciada no exato instante em que um consumidor invoca a função de coleta (`collect`).
...
- **Valor Inicial:** Todo StateFlow requer a passagem de um valor inicial obrigatório.
